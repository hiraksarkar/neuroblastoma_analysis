...
DEheatmap=function(p2myeloid,conosCluster,samp = NULL,appname,removeGene=NULL,num=20,height=9,width=7){
 ...
 if (!is.null(samp)){ 
  annot <- data.frame(CellType=conosCluster[colnames(x)],sample=samp[colnames(x)],row.names = colnames(x))
}else{
annot <- data.frame(CellType=conosCluster[colnames(x)],row.names = colnames(x))
  ...
  rgb.palette <- colorRampPalette(c("blue","white","red"), space = "rgb" )
  heat=pheatmap::pheatmap(x,cluster_cols=FALSE,annotation_col = annot2,show_colnames = F,annotation_legend = TRUE, #,gaps_col =  1,
                cluster_rows = FALSE,color=rgb.palette(100),filename=fout,fontsize_row =5,width=width,height=height,   #3.5*0.02*length(markers),
