...
def heatmap(adata, var_names, groupby=None, use_raw=None, log=False, num_categories=7,
            ...
            show_gene_labels=None, show=None, save=None, figsize=None, srtByDndrgm=False,
            sgmntPlt=None,fig=None,**kwds):
    """\
    ...
    **kwds : keyword arguments
    ...
    if groupby is None or len(categories) <= 1:
        ...
        if groupby + "_colors" in adata.uns:
            ...
            groupby_colors = None
    #######
    #srtByDndrgm Add by Oscar Bedoya-Reina on 19.10.2019 to plot in order excluding the tree
    #######
    if dendrogram or srtByDndrgm:
        dendro_data = _reorder_categories_after_dendrogram(adata, groupby, dendrogram,
                                                           var_names=var_names,
                                                           var_group_labels=var_group_labels,
                                                           var_group_positions=var_group_positions)
    ...
    if not swap_axes:
        ...
        #######
        #srtByDndrgm Add by Oscar Bedoya-Reina on 19.10.2019 to plot in include the figure
        #######      
        #Plot
        if sgmntPlt is None:
            fig = pl.figure(figsize=(width, height))
            ...
            axs=gridspec.GridSpecFromSubplotSpec(nrows=2,ncols=4,width_ratios=width_ratios, \
            wspace=0.15 / width,hspace=0.13 / height, height_ratios=height_ratios, \
            subplot_spec=sgmntPlt)
        ########
        ########
    ...
    # swap axes case
        ...
        if sgmntPlt is None:
            ...
            axs=gridspec.GridSpecFromSubplotSpec(nrows=3, ncols=3, wspace=0.25 / width,
                                hspace=0.3 / height,
                                width_ratios=width_ratios,
                                height_ratios=height_ratios,subplot_spec=sgmntPlt)
        ########
        ########
...
#######
#Next method added by Oscar Bedoya-Reina on 19.10.2019 to plot in include the figure
#######   
@doc_params(show_save_ax=doc_show_save_ax, common_plot_args=doc_common_plot_args)
def heatmapAdjstd(adata, var_names, groupby=None, use_raw=None, log=False, num_categories=7,
            dendrogram=False, gene_symbols=None, var_group_positions=None, var_group_labels=None,
            var_group_rotation=None, layer=None, standard_scale=None, swap_axes=False,
            show_gene_labels=None, show=None, save=None, figsize=None, srtByDndrgm=False,
            sgmntPlt=None,fig=None,labelsize='small',**kwds):
    """\
    Heatmap of the expression values of genes.
    ...
    **kwds : keyword arguments
    ...
    # swap axes case
        ...
        dendro_height = 0.8 if dendrogram else 0.05
        groupby_height = 0.03 if categorical else 0.20
        if figsize is None:
            #if show_gene_labels:
            #    heatmap_height = len(var_names) * 0.18
            #else:
        ...
        height_ratios = [groupby_height, heatmap_height, dendro_height]
        ...
        if sgmntPlt is None:
            fig = pl.figure(figsize=(width, height))
            axs = gridspec.GridSpec(nrows=3, ncols=2, wspace=0.25 / width,
                                hspace=0.3 / height,
                                width_ratios=width_ratios,
                                height_ratios=height_ratios)
        else:
            axs=gridspec.GridSpecFromSubplotSpec(nrows=3, ncols=1, #wspace=0.25 / width,
                                #hspace=0.3 / height,
                                # ~ width_ratios=width_ratios,
                                height_ratios=height_ratios,subplot_spec=sgmntPlt)
        ########
        ...
        heatmap_ax.grid(False)
        if show_gene_labels:
            heatmap_ax.tick_params(axis='y', labelsize=labelsize, length=1)
            heatmap_ax.set_yticks(np.arange(len(var_names)))
            heatmap_ax.set_yticklabels(var_names, rotation=0)
        else:
            heatmap_ax.tick_params(axis='y', labelleft=False, left=False)
        ...
        if categorical:
            groupby_ax = fig.add_subplot(axs[0, 0])
            ticks, labels, groupby_cmap, norm = _plot_categories_as_colorblocks(groupby_ax, obs_tidy, colors=groupby_colors,
                                                                          orientation='bottom',shwLbl=False,shrtnLbl=True)
            # add lines to main heatmap
            line_positions = np.cumsum(obs_tidy.index.value_counts(sort=False))[:-1]
            heatmap_ax.vlines(line_positions, -1, len(var_names) + 1, lw=0.5)
        ...
        # plot group legends next to the heatmap_ax (if given)
        # ~ if var_group_positions is not None and len(var_group_positions) > 0:
            # ~ gene_groups_ax = fig.add_subplot(axs[2, 0])
            # ~ arr = []
            # ~ for idx, pos in enumerate(var_group_positions):
                # ~ arr += [idx] * (pos[1]+1 - pos[0])
        ...
        # plot colorbar
        _plot_colorbar(im, fig, axs[2, 0], max_cbar_height=30,orientation="horizontal")#
    #######
    #srtByDndrgm Add by Oscar Bedoya-Reina on 19.10.2019 to plot in include the figure
    #######      
    #Plot
...
def _plot_categories_as_colorblocks(groupby_ax, obs_tidy, colors=None, orientation='left', cmap_name='tab20',shwLbl=True,shrtnLbl=False):
    """
    ...
    if orientation == 'left':
        ...
        if len(labels) > 1:
            if shwLbl:
                ...
                for l in labels:
                    if len(l)>4:
                        outLbls=''.join([str(v[0]) for v in l.split('_') if v.strip()])
                    else:
                        outLbls=str(l)
                    lLbls.append(outLbls)
                groupby_ax.set_yticklabels(lLbls)

        if shwLbl or shrtnLbl:
            # remove y ticks
            groupby_ax.tick_params(axis='y', left=False, labelsize='small')
        else:
			# remove y ticks and labels
            groupby_ax.tick_params(axis='y', left=False, labelsize=False)
        # remove x ticks and labels
            ...
            if shwLbl:
                ...
                for l in labels:
                    if len(l)>4:
                        ...
                        outLbls=str(l.replace('_',' '))
                    lLbls.append(outLbls)
                groupby_ax.set_xticklabels(lLbls)
        ...
        if shwLbl or shrtnLbl:
			...
			# remove x ticks and labels
            groupby_ax.tick_params(axis='x', bottom=False, labelsize=False)
...
def _plot_colorbar(mappable, fig, subplot_spec, max_cbar_height=4, orientation="vertical"):
    """
    ...
    pl.colorbar(mappable, cax=heatmap_cbar_ax, orientation=orientation)
